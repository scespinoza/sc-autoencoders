...
class Encoder(layers.Layer):
    ...
    def __init__(self, original_dim=5491, latent_dim=10, name='Encoder'):
        ...
        self.logvar_dense = layers.Dense(latent_dim, activation='linear', kernel_initializer=0)
...
class VariationalDeepEmbedding(tf.keras.Model):
    ...
    def vade_loss(self, inputs):
        ...
        log_p_c = tf.reduce_mean(gamma * tf.math.log(p_c + 1e-10))
        log_q_c_given_x = tf.reduce_mean(gamma * tf.math.log(gamma + 1e-10))
        log_q_z_given_x = 0.5 * tf.reduce_mean(1 + logvar)
...
class PrintLossAndAccuracy(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        p_z_c = tf.exp(h) + 1e-10
        print(p_z_c / p_z_c.numpy().sum(1, keepdims=True))"""
